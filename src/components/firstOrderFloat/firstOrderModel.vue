...
<!-- 首单全免 支付弹框 -->
  <div class="alertMain">
    <div class="alertContent" v-if="flag">
      <div class="firstOrderLine ft18 color_white_1">本单立享全额返还 剩余时间 {{dataTime.date_m+':'+dataTime.date_s+':'+dataTime.date_ms}}</div>
      <!-- 内容盒子 -->
        ...
        <div class="leftGameInfo">
          ...
          <div class="texts text5">
            ...
            <div class="checkMarkBox">
              <!-- <div class="checkMark checkMark1 opc_hover8">
  ...
  methods: {
    ...
    createOrder() {
      ...
      var datas = {
        ...
        activityId: this.ifModelType.activityId
      };
      _this.orderDatas = datas;
      console.log(datas.channelId);
      _this.$fetch("/activity/create", _this.orderDatas).then(response => {
        // _this.userBalance = response.data.balance; //用户余额
        _this.tobePaid = response.data.unpay; //待付款金额
        _this.qrcodeUrl = response.data.qrcode; //二维码
        _this.enough = response.data.enough; //是否可以使用余额直接支付
        ...
        // 订单创建完成后调用定时器函数 判断能否参加活动 并且获取时间
        _this.getFirstOrderTime()
      });
    },
    //   使用余额直接支付
    balancePayment() {
