    ...
    <div class="alertContent" v-if="flag">
      ...
      <div class="mainConment" v-if="!ifpayments">
        <div class="leftGameInfo">
          <!-- 图片 -->
          <div class="gameInfoImg newGiftBag_bg">
            <!-- <img class="imgBlock" src="" alt /> -->
          </div>
          <!-- 游戏名称 -->
          <div class="texts text1">
            <div class="ft14 color_white_04">游戏名称</div>
            <div class="ft14 color_white_08">新人大礼包</div>
          </div>
          <!-- 购买类型 -->
          ...
          <!-- 账户余额 -->
          <div class="texts text5" v-if="false">
            <div class="ft14 color_white_04">账户余额</div>
            <div class="checkMarkBox">
              <div class="checkMark checkMark1 opc_hover8" v-if="userBalance > -1">
                <svg-icon v-if="isBalancePayment" iconClass="whiteHookUp" class="svg_icon"></svg-icon>
              </div>
              <div class="ft14 color_white_08 margin_left_8">使用 ¥没有余额</div>
            </div>
          </div>
          <!-- 线条 -->
          <div class="lineBox"></div>
        ...
        <div class="rightPaymentMode">
          ...
          <div v-if="!enough" class="paymentMain">
            <div class="paymentContentTop">
              <div class="wxBox" @click="wxActiveFun">
                <svg-icon
                  :iconClass="activityOrderData.payType == 2 ? 'wxActive' : 'wxDefault'"
                  class="svg_icon"
                ></svg-icon>
              </div>
              <div class="zfbBox" @click="zfbActiveFun">
                <svg-icon
                  :iconClass="activityOrderData.payType == 3 ? 'zfbActive' : 'zfbDefault'"
                  class="svg_icon"
                ></svg-icon>
              </div>
            </div>
          ...
          <div class="lineBox"></div>
          <!-- 卖点 独享 -->
          <!-- <div class="sellingPoint" v-if="activityOrderData.model == 1">
      ...
      <div class="paymentSuccess" v-if="ifpayments">
        ...
        <div class="paymentSuccessBtn ft14 color_white_1" @click="gotoMyGames">点击跳转我的游戏</div>
      </div>
    </div>
...
export default {
  ...
  props: ["activityOrderData",'activityAlertModelChange'],
  ...
  methods: {
    ...
    createOrder() {
        var _this = this;
        console.log(_this.activityOrderData)
        _this.$fetch('/people/activity/create',_this.activityOrderData)
            .then((response) => {
                ...
                _this.discount = response.data.discount; //优惠金额
                _this.worth = response.data.worth; //价值
                //创建订单完成开始轮询 查询订单是否完成
                _this.setTimerAjax();
                //生成二维码失效时间定时器
      ...
      _this
        ...
        .then(response => {
          if (response.flag) {
            ...
            // _this.ifpayments = true;  "支付成功  跳转成功页面"
            ...
            // _this.$listeners.change(true,1)//支付成功 告诉爷爷    触发父组件方法的另一种方式  
            _this.$emit("changes", false,'closeParent');
          } else {
            // clearInterval(_this.timer);
            // _this.timer = null;
    ...
    refreshQrcodeUrl() {
      ...
      clearInterval(_this.qrcodeSetTimeOut);
      _this.qrcodeSetTimeOut = null;
      _this.$fetch("/people/activity/create", _this.activityOrderData).then(response => {
       // _this.userBalance = response.data.balance; //用户余额
        _this.tobePaid = response.data.unpay; //待付款金额
        _this.qrcodeUrl = response.data.qrcode; //二维码
        _this.enough = response.data.enough; //是否可以使用余额直接支付
    ...
    zfbActiveFun() {
      var _this = this;
      if(_this.activityOrderData.payType != 3){
         _this.activityOrderData.payType = 3;
    ...
    wxActiveFun() {
      var _this = this;
      if(_this.activityOrderData.payType != 2){
        _this.activityOrderData.payType = 2;
    ...
    // 打开协议
    openUrl() {
    //   if (this.activityOrderData.model == 1) {
        //独享
        // var urls = window.location.protocol + '//' + window.location.host + "/duxiang.html";
