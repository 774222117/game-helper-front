...
<!-- 首页弹窗 -->
    <div class="newyearalert" v-if="loginState == true &&  dailyLogin == true ? true : false" @click="closeAlert(false)">
        ...
        <div class="redBagBox">
            ...
            <!-- 红包 -->
            <div class="envelopes redBag_bg"></div>
            <!-- 免费领取游戏 -->
            <div class="getGame getFreeGame_bg" :class=" item ?'gotosee_bg':'getFreeGame_bg'" @click="goToNewYear">
            </div>
        </div>
    </div>
...
export default {
    ...
    methods:{
        ...
        goToNewYear(){
            ...
            var loginInfo =  getStore({name:'loginNewYear',type:false})
            if(loginInfo.value && loginInfo.active == 0){//如果 用户已经参加活动 && 用户当前没有点击'去看看'
                ...
                setStore({name:'loginNewYear',content:loginInfo,type:false})
            }
            JumpTo(22);
...
@import "../../assets/newYear/css/homepage.css";
