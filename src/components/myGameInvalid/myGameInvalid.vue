      ...
      <div class="mainConment" v-if="!ifpayments">
        <div class="leftGameInfo">
          ...
          <div class="gameInfoImg">
            <img class="imgBlock" :src="myGameInvalid.image" alt />
          </div>
          <!-- 游戏名称 -->
          <div class="texts text1">
            <div class="ft14 color_white_04">游戏名称</div>
            <div class="ft14 color_white_08">{{ myGameInvalid.chinaName == '' ? myGameInvalid.englishName : myGameInvalid.chinaName}}</div>
          </div>
          <!-- 购买类型 -->
          <div class="texts text2">
            <div class="ft14 color_white_04">购买类型</div>
            <div class="ft14 color_white_08">{{myGameInvalid.model == 1?'独享版':'畅享版' }}</div>
          </div>
          <!-- 线条 -->
        ...
        <div class="rightPaymentMode">
          ...
          <div
            ...
            class="balancePayment ft14 color_white_1"
          ...
          <div class="lineBox"></div>
          <!-- 卖点 独享 -->
          <div class="sellingPoint" v-if="myGameInvalid.model == 1">
          ...
          <!-- 卖点 畅享版 -->
          <div class="sellingPoint" v-if="myGameInvalid.model == 2">
              <div class="sellingPointItems">
                <div class="iconBox"><svg-icon iconClass='gift' className='svg_icon'></svg-icon></div>
                <div class="textBox ft12 color_white_06">官方正版</div>
              </div>
...
export default {
  ...
  components: { JumpTo, loading ,store,post},
  props: ["myGameInvalid"],
  data() {
    return {
      flag: false, //接口请求完成
  ...
  methods: {
    ...
    balancePaymentHook() {
      ...
      _this.$fetch("/people/activity/buyOverdue", _this.orderDatas)
        .then(response => {
    ...
    createOrder() {
      ...
      console.log(_this.myGameInvalid)
      _this.orderDatas = datas
      _this.$fetch("/people/activity/buyOverdue", _this.orderDatas).then(response => {
        _this.userBalance = response.data.balance; //用户余额
        _this.tobePaid = response.data.unpay; //待付款金额
        _this.qrcodeUrl = response.data.qrcode; //二维码
        _this.enough = response.data.enough; //是否可以使用余额直接支付
      ...
      _this.$fetch("/order/balance/pay", {'orderId':_this.orderId})
        .then(response => {
          if (response.flag) {
            ...
            this.setTimer();
            this.$emit("change", false);  //支付成功  关闭
          }
        });
    },
    //查询订单状态
